...
public class TabAPI {
    ...
    public static void clearTabLists() {
        for (TabList tabList : tabLists.values()) {
            try {
                ProtocolLibrary.getProtocolManager().sendServerPacket(tabList.getOwner(), tabList.getRemovePacket());
    ...
    public static void loadData(final TabCellShape[] tabCellShapeMap, String header, String footer) {
        ...
        headerAndFooterPacket.getChatComponents().write(0, WrappedChatComponent.fromText(StringUtil.fixColors(header)));
        headerAndFooterPacket.getChatComponents().write(1, WrappedChatComponent.fromText(StringUtil.fixColors(footer)));
    }
