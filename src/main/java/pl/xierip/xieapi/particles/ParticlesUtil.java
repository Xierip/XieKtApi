...
public class ParticlesUtil {
  ...
  public static void effectTrippleCircle(Location location, int range, ParticleData particlePacket1,
      ParticleData particlePacket2, ParticleData particlePacket3, JavaPlugin plugin) {
    new BukkitRunnable() {
      @Override
      public void run() {
        ...
        for (Entity entity : entities) {
          ...
          for (int i = 0; i < 360; i += 5) {
            ...
            ((CraftPlayer) near).getHandle().playerConnection.networkManager.sendPacket(
                particlePacket1
                    .toPacket((float) circle.getX(), (float) circle.getY(), (float) circle.getZ()));
            ((CraftPlayer) near).getHandle().playerConnection.networkManager.sendPacket(
                particlePacket2.toPacket((float) circle2.getX(), (float) circle2.getY(),
                    (float) circle2.getZ()));
            ((CraftPlayer) near).getHandle().playerConnection.networkManager.sendPacket(
                particlePacket3.toPacket((float) circle3.getX(), (float) circle3.getY(),
                    (float) circle3.getZ()));
