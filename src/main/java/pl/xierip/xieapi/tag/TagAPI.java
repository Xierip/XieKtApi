...
import pl.xierip.xieapi.packet.PacketUtil;
...
public class TagAPI {
  ...
  public static void updateTagAndSuffix(Player player, Player target, String tag, String suffix) {
    ...
    if (!map.containsKey(name)) {
      AsyncPacketThread
          .addPacket(target, PacketUtil.buildTeamPacket(name, name, tag, suffix, 0, name));
      map.put(name, tag);
    } else {
      AsyncPacketThread
          .addPacket(target, PacketUtil.buildTeamPacket(name, name, tag, suffix, 2, name));
      map.put(name, tag);
    }
  }
}
