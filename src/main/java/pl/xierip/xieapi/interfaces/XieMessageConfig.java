...
import pl.xierip.xieapi.objects.messages.XieMessageActionBar;
import pl.xierip.xieapi.objects.messages.XieMessageBossBar;
import pl.xierip.xieapi.objects.messages.XieMessageText;
import pl.xierip.xieapi.objects.messages.XieMessageTitle;
...
public interface XieMessageConfig extends XieMessage {
    ...
    public static <E extends Enum<E> & XieMessageConfig> void loadAndSortConfig(Class<E> eClass, String path, Config config, Logging logging) {
        ...
        if (exception) {
            try {
                String newName = "broken_" + config.getFile().getName() + "_" + System.currentTimeMillis() + ".yml";
                FileUtils.copyFile(config.getFile(), new File(config.getFile().getParentFile(), newName));
                logging.log(LogType.INFO, "[CONFIG] Making copy of: " + config.getFile().getName() + ", new name: " + newName+"");
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    public static void save(String path, Config config, XieMessage message, boolean force) {
        if (!force && config.isSet(path)) return;
        config.set(path, null);
    ...
    public static XieMessage parseMessage(String path, Config config) throws ConfigException {
        try {
            ...
            switch (xieMessageType) {
                case TEXT:
                    return new XieMessageText(config.isInstanceOf(path + ".text", String.class) ? config.getString(path + ".text") : StringUtils.join(config.getStringList(path + ".text"), "\n"));
                case BOSSBAR:
                    return new XieMessageBossBar(config.getString(path + ".text"), config.getInt(path + ".stay"), BarStyle.valueOf(config.getString(path + ".style").toUpperCase()), BarColor.valueOf(config.getString(path + ".color").toUpperCase()));
                case ACTIONBAR:
                    return new XieMessageActionBar(config.getString(path + ".text"), config.getInt(path + ".stay"));
                case TITLE:
                    return new XieMessageTitle(config.getString(path + ".title"), config.getString(path + ".subtitle"), config.getInt(path + ".in"), config.getInt(path + ".stay"), config.getInt(path + ".out"));
            }
        } catch (IllegalArgumentException e) {
            throw new ConfigException("'" + path + "' Cannot find enum with this name (" + e.getMessage() + ")");
        }
        return null;
    }
}
