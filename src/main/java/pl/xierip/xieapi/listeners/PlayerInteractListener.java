...
public class PlayerInteractListener implements Listener {
  ...
  private void onInteract(final PlayerInteractEvent event) {
    ...
    Bukkit.getPluginManager().callEvent(new ButtonClickEvent(event.getPlayer(),
        event.getClickedBlock()
            .getRelative(((Button) event.getClickedBlock().getState().getData()).getAttachedFace()),
        (Button) event.getClickedBlock().getState().getData()));
  }
}
