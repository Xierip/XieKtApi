...
public class EntityDamageByEntityListener implements Listener {
    ...
    private void onDMG(final EntityDamageByEntityEvent event) {
        ...
        if (event.getEntity() instanceof Player) {
            ...
            if (damager == null) {
                if (event.getDamager() instanceof Projectile) {
                    if (((Projectile) event.getDamager()).getShooter() instanceof Player) {
                        ...
                        attackType = event.getDamager() instanceof Arrow ? AttackType.ARROW : AttackType.OTHER_PROJECTILE;
                    } else {
                        return;
                    }
                } else {
            ...
            if (damager != null) {
                final PlayerDamageByPlayerEvent playerDamageByPlayerEvent = new PlayerDamageByPlayerEvent(p, damager, event.getDamage(), event.isCancelled(), attackType, projectile);
