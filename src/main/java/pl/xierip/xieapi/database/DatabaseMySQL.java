...
public class DatabaseMySQL {
  ...
  public DatabaseMySQL(final String mysqlHOST, final String mysqlDATABASE, final String mysqlUSER,
    ...
    try {
      ...
      XieAPI.getLogging().log(LogType.INFO,
          "Connected to database with &c" + (System.currentTimeMillis() - time) + "ms&r");
  ...
  public void update(final String sql) {
    if (sql.startsWith("SELECT")) {
      XieAPI.getLogging()
          .log(LogType.ERROR, "MYSQL: SELECT cannot be used in update method {" + sql + "}");
    } else {
      queueThread.getQueue().add(sql);
    }
  }
