...
import pl.xierip.xieapi.interfaces.Command;
import pl.xierip.xieapi.objects.CommandCooldown;
import pl.xierip.xieapi.XieAPI;
import pl.xierip.xieapi.enums.LogType;
import pl.xierip.xieapi.enums.SenderType;
import pl.xierip.xieapi.interfaces.XieCommandStructure;
import pl.xierip.xieapi.interfaces.XiePlayerCommand;
...
public class CommandsManager extends BukkitCommand {
    ...
    public static void registerCommand(final Plugin plugin, final XieCommandStructure handler) {
        for (final Method method : handler.getClass().getDeclaredMethods()) {
            ...
            if (value.length != 2 || !(CommandSender.class.equals(value[0]) || Player.class.equals(value[0])) || !String[].class.equals(value[1])) {
                continue;
            }
    ...
    public static void registerCommands(final Plugin plugin, final XieCommandStructure... xieCommands) {
    ...
    public boolean execute(final CommandSender commandsender, final String label, final String[] args) {
        for (final Map.Entry<XieCommandStructure, Method> listeners : COMMANDS.entrySet()) {
            ...
            if (!cmd.permission().equals("") && !commandsender.hasPermission(cmd.permission())) {
                SendUtil.sendMessage(commandsender, StringUtil.replace(cmd.permissionMessage(), "{PERMISSION}", cmd.permission()));
                return true;
            }
            ...
            if (commandsender instanceof Player) {
                ...
                if ((!player.hasPermission("xiektapi.bypass.cooldown." + cmd.name())) && cooldowns.containsKey(key)) {
                    final CommandCooldown commandCooldown = cooldowns.get(key);
                    if (!commandCooldown.addCooldown(player.getUniqueId(), args)) {
                        SendUtil.sendMessage(commandsender, "&cBedziesz mogl uzyc tej komendy za: " + DateUtil.formatTimeTo(commandCooldown.getCooldown(player.getUniqueId(), args)));
                        return true;
                    }
