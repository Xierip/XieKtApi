package pl.xierip.xieapi.parsers;
...
import pl.xierip.xieapi.utils.CollectionsUtil;
...
public class Parsers {
  ...
  public static ItemStack parseItemStack(final String itemStack) {
    ...
    for (int i = 2; i < strings.length; i++) {
      ...
      if (trim[0].equalsIgnoreCase("lore")) {
        final ItemMeta im = is.getItemMeta();
        im.setLore(StringUtil.fixColors(
            CollectionsUtil.toList(StringUtil.replace(trim[1], "_", " ", ";", ":").split("&nl"))));
        is.setItemMeta(im);
        continue;
      }
    ...
    if (signatureTexture != null && valueTexture != null) {
      try {
        ...
        XieAPI.getLogging()
            .log(LogType.EXCEPTION, "Error with parsing head, cannot set profile", e);
      }
    }
    return is;
  }
  ...
  public static PotionEffect parsePotionEffect(final String string) {
    ...
    return new PotionEffect(PotionEffectType.getById(parsedInts[0]),
        (parsedInts.length > 2 ? parsedInts[2] : 10) * 20, parsedInts[1]);
  }
