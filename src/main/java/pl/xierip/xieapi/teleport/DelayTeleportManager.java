...
/**
 * Created by xierip on 12.05.18. Web: http://xierip.pl
 */
public class DelayTeleportManager {
  ...
  public static boolean teleport(final Player player, final Location target, final int time,
      final XieMessage before, final XieMessage after) {
  ...
  public static boolean teleport(final Player player, final Player target, final int time,
      final XieMessage before, final XieMessage after) {
    if (teleports.containsKey(player)) {
      SendUtil.sendMessage(player, "&cPosiadasz oczekujaca teleportacje!");
      return false;
