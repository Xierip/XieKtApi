...
import pl.xierip.xieapi.utils.EffectsUtil;
...
public class DelayTeleport {
    ...
    public DelayTeleport(final Player p, final Location to, final int time, final XieMessage before, final XieMessage after) {
        this.player = p;
        this.to = null;
        AtomicInteger counter = new AtomicInteger();
        this.player.addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, time * 40, 0));
        before.sendMessage(p);
        this.task = Bukkit.getServer().getScheduler().scheduleSyncRepeatingTask(XieAPI.getInstance(), () -> {
            final int i = counter.get();
            EffectsUtil.effectTrippleCircle(this.player.getLocation(), 30, new ParticleData(EnumParticle.REDSTONE, false, 1, 1, 1, 1, 0), new ParticleData(EnumParticle.REDSTONE, false, 1, 1, 1, 1, 0), new ParticleData(EnumParticle.REDSTONE, false, 1, 1, 1, 1, 0), XieAPI.getInstance());
            this.player.sendActionBar(StringUtil.fixColors("&a" + StringUtils.repeat('\u2588', i) + "&c" + StringUtils.repeat('\u2588', time - i)));
            if (i == time) {
    ...
    public DelayTeleport(final Player p, final Player to, final int time, final XieMessage before, final XieMessage after) {
        this.player = p;
        this.to = to;
        AtomicInteger counter = new AtomicInteger();
        this.player.addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, time * 40, 0));
