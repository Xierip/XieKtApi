...
/**
 * Created by Xierip on 01.02.2017.
 * Web: http://xierip.pl
 */
public class CommandCooldown {
    ...
    public boolean addCooldown(final UUID uuid, final String[] args) {
        ...
        if (!cooldownPlayers.containsColumn(uuid)) {
            cooldownPlayers.put(cooldownType, uuid, System.currentTimeMillis() + cooldowns.get(cooldownType));
            return true;
    ...
    public void clean() {
        if (cooldown != 0L) {
            if (cooldownPlayersSoft.isEmpty()) return;
            ...
            for (Map.Entry<String[], Map<UUID, Long>> entry : new HashMap<>(cooldownPlayers.rowMap()).entrySet()) {
    ...
    private String[] findCooldownByArgs(final String[] args) {
        ...
        for (final String[] strings : cooldownList) {
            if (strings.length > args.length) continue;
            boolean all = true;
