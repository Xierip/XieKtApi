...
public class ConfigManager {
  ...
  private static Config register(final String id, String resource, String path,
    ...
    cfg.load();
    if (CONFIGS.stream().filter(config -> config.getId().equals(id)).findFirst().orElse(null)
        != null) {
      return cfg;
    }
    CONFIGS.add(cfg);
