    ...
    public static List<ItemStack> getDrops(final Block block, final ItemStack item) {
        ...
        switch (type) {
            ...
            case SKULL:
                ...
                ItemStack itemStack1 = new ItemStack(Material.SKULL_ITEM, 1, (short) skull.getSkullType().ordinal());
            ...
            case NETHER_WARTS: {
                ...
                amount = (warts.getState() == NetherWartsState.RIPE ? (RandomUtil.getIntBetween(0, 2) + 2) : 1);
            ...
            case LEAVES_2:
                if (item.getType() == Material.SHEARS || item.containsEnchantment(Enchantment.SILK_TOUCH)) {
                    items.add(new ItemStack(block.getType(), 1, (short) (((Leaves) block.getState().getData()).getSpecies().getData() - (block.getType() == Material.LEAVES_2 ? 4 : 0))));
                    break;
                }
                ...
                if (RandomUtil.getChance(8)) {
                    items.add(new ItemStack(Material.SAPLING, 1, ((Leaves) block.getState().getData()).getSpecies().getData()));
                }
                break;
            case DOUBLE_PLANT:
            case REDSTONE_WIRE:
